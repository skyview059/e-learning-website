...
if($f_pending_test_result_ids > 0 && (($f_pending_test_ids == $f_testid) || ($f_testid == 0))) {
 ...
 $i_rSet1 = $g_db->SelectLimit("SELECT * FROM ".$srv_settings['table_prefix']."results WHERE resultid=".$f_pending_test_result_ids." AND id=".$G_SESSION['id']." AND gscale_gradeid=0", 1);
if(!$i_rSet1) {
 showDBError(__FILE__, 1);
} else {
 if(!$i_rSet1->EOF) {
