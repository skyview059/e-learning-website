...
$i_sql_order_addon = '';
$i_tablefields = array(
	array($lngstr['label_manageusers_hdr_id'], $lngstr['label_manageusers_hdr_id_hint'], $srv_settings['table_prefix'].'users.id'),
	array($lngstr['label_manageusers_hdr_user_notes'], $lngstr['label_manageusers_hdr_user_notes_hint'], ''),
	array($lngstr['label_manageusers_hdr_username'], $lngstr['label_manageusers_hdr_username_hint'], $srv_settings['table_prefix'].'users.username'),
	array($lngstr['label_manageusers_hdr_email'], $lngstr['label_manageusers_hdr_email_hint'], $srv_settings['table_prefix'].'users.email'),
...
$i_rSet1 = $g_db->SelectLimit("SELECT id, username, email, user_firstname, user_lastname, user_department, user_notes, user_enabled FROM ".$srv_settings['table_prefix']."users".(!empty($i_sql_where_addon) ? ' WHERE '.$i_sql_where_addon : '').$i_sql_order_addon, $i_limitcount, $i_limitfrom);
if(!$i_rSet1) {
...
echo '<tr id=tr_'.$i_pagewide_id.' class='.$rowname.' onmouseover="rollTR('.$i_pagewide_id.',1);" onmouseout="rollTR('.$i_pagewide_id.',0);"><td align=center width=22'.($i_rSet1->fields['id'] > SYSTEM_USER_MAX_INDEX ? '' : ' class=system').'><input id=cb_'.$i_pagewide_id.' type=checkbox name=box_users[] value="'.$i_rSet1->fields['id'].'" onclick="toggleCB(this);"></td><td align=right>'.$i_rSet1->fields['id'].'</td><td align=center width=16 style="padding: 1px;"><a href="javascript:void(0)" onClick="showDialog(\'users.php?id='.$i_rSet1->fields['id'].'&action=notes\', 300, 200)"><img src="images/button-notes.gif" width=16 height=20 title="'.convertTextValue($i_rSet1->fields['user_notes']).'" border=0></a></td><td>'.$i_rSet1->fields['username'].'</td><td>'.$i_rSet1->fields['email'].'</td><td>'.$i_rSet1->fields['user_firstname'].'</td><td>'.$i_rSet1->fields['user_lastname'].'</td><td align=center><a href="users.php?id='.$i_rSet1->fields['id'].$i_order_addon.$i_url_limit_addon.'&action=enable&set='.($i_rSet1->fields['user_enabled'] ? '0"><img src="images/button-checkbox-2.gif" width=13 height=13 border=0 title="'.$lngstr['label_yes'].'">' : '1"><img src="images/button-checkbox-0.gif" width=13 height=13 border=0 title="'.$lngstr['label_no'].'">').'</a></td>';
echo '<td align=center width=22><a href="users.php?id='.$i_rSet1->fields['id'].'&action=groups"><img width=20 height=20 border=0 src="images/button-groups.gif" title="'.$lngstr['label_action_manageusers_groups'].'"></a></td><td align=center width=22><a href="users.php?id='.$i_rSet1->fields['id'].'&action=edit"><img width=20 height=20 border=0 src="images/button-edit.gif" title="'.$lngstr['label_action_manageusers_edit'].'"></a></td><td align=center width=22>'.($i_rSet1->fields['id'] > SYSTEM_USER_MAX_INDEX ? '<a href="users.php?id='.$i_rSet1->fields['id'].'&action=delete" onclick="return confirmMessage(this, \''.$lngstr['qst_delete_user'].'\')"><img width=20 height=20 border=0 src="images/button-cross.gif" title="'.$lngstr['label_action_manageusers_delete'].'"></a>' : '<img width=20 height=20 border=0 src="images/button-cross-inactive.gif">').'</td></tr>';
$i_counter++;
$i_pagewide_id++;
$i_rSet1->MoveNext();
}
