...
-- \$_SESSION['AVAR']={$_SESSION['AVAR']}</p>";
 ...
 1. Create this table in your database (syntax might vary depending on your db):
  ...
  4. If you want to receive notifications when a session expires, then
	 ...
	 has expired, so we store the user id in the global variable $id, 
	 store this value in the EXPIREREF field:
	 
	 	$ADODB_SESSION_EXPIRE_NOTIFY = array('id','NotifyFn');
		
	Then when the NotifyFn is called, we are passed the $id as the first
	parameter, eg. NotifyFn($id, $sesskey).
*/
