...
if(!empty($_SERVER['QUERY_STRING']))
 ...
 $i_startdate = time();
$i_id = isset($G_SESSION['id']) ? $G_SESSION['id'] : 0;
...
$i_ip1 = isset($i_ip_nets[0]) ? (int)$i_ip_nets[0] : 0;
$i_ip2 = isset($i_ip_nets[1]) ? (int)$i_ip_nets[1] : 0;
$i_ip3 = isset($i_ip_nets[2]) ? (int)$i_ip_nets[2] : 0;
$i_ip4 = isset($i_ip_nets[3]) ? (int)$i_ip_nets[3] : 0;
...
if($g_db->Execute("INSERT INTO ".$srv_settings['table_prefix']."visitors (startdate, id, ip1, ip2, ip3, ip4, host, referer, useragent, inurl, outurl) VALUES ($i_startdate, $i_id, $i_ip1, $i_ip2, $i_ip3, $i_ip4, ".$i_host.", ".$i_referer.", ".$i_useragent.", ".$i_inurl.", ".$i_outurl.")")===false) {
 $i_visitorid = 0;
} else {
 $i_visitorid = (int)$g_db->Insert_ID($srv_settings['table_prefix'].'visitors', 'visitorid');
}
