...
$i_duplicates = getRecordCount($srv_settings['table_prefix'].'users', "username=".$g_db->qstr($i_username, get_magic_quotes_gpc()));
...
if(!empty($arrUserData)) {
 if(isset($i_id)) {
 $qry_str = 'UPDATE '.$srv_settings['table_prefix'].'users SET '.$qry_str.' WHERE id='.$i_id;
$g_db->Execute($qry_str);
return true;
} else if(isset($arrUserData['username'])) {
 $qry_str = 'UPDATE '.$srv_settings['table_prefix'].'users SET '.$qry_str.' WHERE username='.$g_db->qstr($arrUserData['username'], get_magic_quotes_gpc());
$g_db->Execute($qry_str);
return true;
}
}
...
function manageTestPurchase($id, $testid, $purchase = true) {
global $g_db, $srv_settings;
if($purchase)
 $g_db->Execute("INSERT INTO ".$srv_settings['table_prefix']."tests_own (testid, id) VALUES (".$testid.", ".$id.")");
else $g_db->Execute("DELETE FROM ".$srv_settings['table_prefix']."tests_own WHERE testid=".$testid." AND id=".$id);
}
...
function manageUserGroups($i_ids = array(), $arrGroupIDs = array(), $i_addtogroup = true) {
global $g_db, $G_SESSION, $srv_settings;
 ...
 if($i_addtogroup) {
 //9917//9917
 $g_db->Execute("INSERT INTO ".$srv_settings['table_prefix']."groups_users (groupid, id) VALUES (".$i_groupid.", ".$i_id.")");
} else {
//9917//9917
 $g_db->Execute("DELETE FROM ".$srv_settings['table_prefix']."groups_users WHERE groupid=".$i_groupid." AND id=".$i_id);
}
}
}
}
function deleteUserByID($i_id = 0) {
global $g_db, $srv_settings;
if($i_id > 0)
 $g_db->Execute("DELETE FROM ".$srv_settings['table_prefix']."users WHERE id=".$i_id);
}
function deleteUserByUserName($i_username = '') {
global $g_db, $srv_settings;
$i_username = $g_db->qstr($i_username, get_magic_quotes_gpc());
$g_db->Execute("DELETE FROM ".$srv_settings['table_prefix']."users WHERE username=".$i_username);
}
function activateUser($i_parameters = array()) {
 $i_activate = isset($i_parameters['activate']) ? $i_parameters['activate'] : true;
if(isset($i_parameters['id'])) {
 activateUserByID($i_parameters['id'], $i_activate);
} else if(isset($i_parameters['username'])) {
 activateUserByUserName($i_parameters['username'], $i_activate);
}
}
function activateUserByID($i_id = 0, $i_activate = true) {
global $g_db, $srv_settings;
if($i_id > 0)
 $g_db->Execute("UPDATE ".$srv_settings['table_prefix']."users SET user_enabled=".($i_activate ? 1 : 0)." WHERE id=".$i_id);
}
function activateUserByUserName($i_username = '', $i_activate = true) {
global $g_db, $srv_settings;
$i_username = $g_db->qstr($i_username, get_magic_quotes_gpc());
$g_db->Execute("UPDATE ".$srv_settings['table_prefix']."users SET user_enabled=".($i_activate ? 1 : 0)." WHERE username=".$i_username);
}
...
if(!$i_rSet1) {
 ...
 $G_SESSION['id'] = $i_rSet1->fields['id'];
$G_SESSION['username'] = $i_rSet1->fields['username'];
$G_SESSION['email'] = $i_rSet1->fields['email'];
...
$i_rSet2->Close();
 ...
 $g_db->Execute("UPDATE ".$srv_settings['table_prefix']."users SET user_logindate=$i_time WHERE id=".$G_SESSION['id']);
 
 if(isset($G_SESSION['visitorid']))
 $g_db->Execute("UPDATE ".$srv_settings['table_prefix']."visitors SET id=".$G_SESSION['id']." WHERE visitorid=".$G_SESSION['visitorid']);
...
function getTestAttemptsUsed($testid, $id) {
global $g_db, $srv_settings;
 
 $i_result = 0;
$i_rSet1 = $g_db->Execute("SELECT test_attempt_count FROM ".$srv_settings['table_prefix']."tests_attempts WHERE testid=".$testid." AND id=".$id);
if(!$i_rSet1) {
 showDBError(__FILE__, 1);
} else {
 if(!$i_rSet1->EOF)
...
function api_manageUserGroups($i_ids = array(), $arrGroupIDs = array(), $i_addtogroup = true) {
 return manageUserGroups($i_ids, $arrGroupIDs, $i_addtogroup);
}
