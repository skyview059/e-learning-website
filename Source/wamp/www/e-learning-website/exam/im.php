...
class dumper {
	...
	function backup() {
		...
		echo tpl_page(tpl_process("A backup DB "), $buttons);
		...
		echo tpl_l("Connecting to the database.");
		...
		echo tpl_l("Creating DB backup file:<BR>\\n  -  {$this->filename}");
	...
	function restore(){
		...
		echo tpl_l("Connecting to the database `{$db}`.");
		mysql_select_db($db) or trigger_error ("Unable to select database. .<BR>" . mysql_error(), E_USER_ERROR);
...
<HTML>
<HEAD>
<TITLE>TanPhu.Net | &copy; 2006 zapimir</TITLE>
...
<B><A HREF=http://TanPhu.Net/ STYLE="color: white; text-decoration: none;">TanPhu.Net</A></B><IMG ID=GS WIDTH=1 HEIGHT=1 STYLE="visibility: hidden;"> Mang Giai Tri TrucTuyen-DownLoad MienPhi-Toc Do Nhanh !</TD>
</TR>
<TR>
...
Backup / Make a database backup &nbsp;</LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD WIDTH=35%>DB:</TD>
...
Restore / Restore a database backup&nbsp;</LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD>DB:</TD>
...
{$error}
<FIELDSET>
<LEGEND>Enter the username and password </LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
