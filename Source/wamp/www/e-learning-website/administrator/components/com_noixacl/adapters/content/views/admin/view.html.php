...
class ContentViewAdmin extends JView
{
	...
	function getSectionsList()
	{
		$db			=& JFactory::getDBO();
                 ...
                 * Get Sections
                 */
		$querySections = "SELECT s.id, s.title, grp.name "
                    . "AS groupname FROM #__sections AS s, #__groups "
                    . "AS grp WHERE s.published = 1 "
                    . "AND s.access = grp.id "
                    . "ORDER BY title";
		...
		foreach($sectionsList as $section){
			$queryCategoryItens = "SELECT cat.*,grp.name "
                            . "AS groupname, cat.parent_id AS parent "
                            . "FROM #__categories AS cat, #__groups AS grp "
                            . "WHERE cat.access = grp.id "
                            . "AND cat.section = {$section->id} "
                            . "AND cat.published = 1";
