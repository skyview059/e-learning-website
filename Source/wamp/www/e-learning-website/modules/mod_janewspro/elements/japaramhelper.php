...
class JElementJaparamhelper extends JElement
{
	...
	function categories ( $name, $value, &$node, $control_name ){
		...
		$query = '
			...
			ORDER BY  s.ordering, s.id, c.ordering';
		$db->setQuery( $query );
		...
		foreach ($cats as $k=>$cat) {					
			if($section_id != $cat->section) {				
				...
				$selecte = '';
				if( (is_array($value) && in_array('sec.'.$section_id, $value)) || (!is_array($value) && 'sec.'.$section_id==$value) ){	
					$selecte = 'selected="selected"';
				}
				$HTMLCats 	.= '<option value="sec.'.$cat->section.'" '.$selecte.'>'.$cat->section_title . ' (ID: '.$cat->section. ')' .'</option>';
			}
							
			$selecte = '';
			if( (is_array($value) && in_array('cat.'.$cat->cat_id, $value)) || (!is_array($value) && 'cat.'.$cat->cat_id==$value) ){	
				$selecte = 'selected="selected"';
			}
			$HTMLCats 	.= '<option value="cat.'.$cat->cat_id.'" '.$selecte.' class="subcat">'.$spacer.$pre.$cat->cat_title. ' (ID: '.$cat->cat_id. ')' .'</option>';
		}
	...
	function group( $name, $value, &$node, $control_name ){
		...
		foreach ($node->option as $option) {
				$str_els = trim((string) $option->_data);
				$str_els = str_replace("\n", '', $str_els);
				$hideRow = isset($option->_attributes['hiderow'])?''.$option->_attributes['hiderow'].'':1;
				$html .= "japh_addgroup ('{$option->_attributes['for']}', { val: '{$option->_attributes['value']}', els_str: '{$str_els}', group:'{$control_name}', hideRow: $hideRow});";
		}
		$html .= '</script>';
