<?php
/**
* @version $Id: menu.menuloader.class.php 517 2011-06-12 12:47:20Z  $
* @author Daniel Ecer
* @package exmenu
...
class ExtendedMenuLoader extends AbstractExtendedMenuLoader {
	...
	function loadMenuItems($sourceValues = array()) {
		...
		if ($this->exactAccessLevel) {
			...
			$sql	.= ' AND access IN ('.implode(',', $this->getUserAccessIds()).')';
		}
		if ($this->loadFirstLevelOnly) {
			if ($siteHelper->isJoomla16()) {
				$sql	.= ' AND parent_id = (SELECT id FROM #__menu WHERE alias = \'root\')';
			} else {
				$sql	.= ' AND parent = 0';
			}
		}
		...
		if ((is_null($rows)) || (is_null($mainRows))) {
			...
			foreach(array_keys($rows) as $id) {
				$row = $rows[$id];
				$parentId	= (isset($row->parent_id) ? $row->parent_id : (isset($row->parent) ? $row->parent : 0));
...
//					// TODO fix
//					$this->showDebug('orphaned sub menu item: '.(isset($row->alias) ? $row->alias : $row->name).' (id='.$id.')');
//					$orphanedIds[]	= $id;
				} else {
					// main level item
					// Note: this now also includes 'orphaned' sub menu items because the newly introduced
					//   'root' entry is not part of the result
					$mainRows[] = $row;
				}
			}
			$i	= 0;
			while($i < count($orphanedIds)) {	// do not use foreach here because we may add items to the array
				...
				if (isset($row->_children)) {
					foreach(array_keys($row->_children) as $key) {
						...
						$this->showDebug('child of orphaned menu item: '.(isset($child->alias) ? $child->alias : $child->name).' (id='.$id.')');
		...
		if (isset($rows[$activeMenuId])) {
			...
			while(($id > 0) && (isset($rows[$id]))) {
				...
				$activeIds[] = $id;
				$id	= (isset($activeRow->parent_id) ? $activeRow->parent_id : $activeRow->parent);
			}
		}
		if (count($sourceValues) > 0) {
			...
			foreach(array_keys($mainRows) as $key) {
				...
				$alias = (isset($menuRow->alias) ? $menuRow->alias : $menuRow->name);
				if ((($menuRow->id > 0) && (in_array($menuRow->id, $sourceValues))) ||
						(($alias != '') && (in_array($alias, $sourceValues)))) {
